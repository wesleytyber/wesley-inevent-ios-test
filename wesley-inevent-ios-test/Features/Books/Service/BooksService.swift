...
class BooksService {
    ...
    func searchBooksPerPage(with string: String, startIndex: String, completion: @escaping (ModelBooks?, NetworkError?) -> Void) {
        Repository.manager.request(API.searchBooksPerPage(code: string, startIndex: startIndex)).response { response in
            guard let data = response.data else { completion(nil, nil) ; return }
            NetworkLogger.log(request: response.request, response: response.response, data: data, error: response.error)
